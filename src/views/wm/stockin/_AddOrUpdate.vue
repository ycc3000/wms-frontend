<template>
	<el-dialog
		...
		:title="!!data.formdata.id ? `修改入库单：${state.stockInNo}` : '新增入库单'"
					...
					<el-row>
						<el-col :span="8">
							<el-form-item prop="businessType" label="业务类型">
								<el-select v-model="data.formdata.businessType">
									...
									<el-option label="采购收货" value="采购收货" />
						...
						<el-col :span="8">
							<el-form-item label="预计到货时间">
								<el-date-picker v-model="data.formdata.estimatedArrivalTime" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD" />
							</el-form-item>
						</el-col>
					</el-row>
					<el-row>
...
const getDefaultFormData = (): EditStockIn => {
	return {
		businessType: '',
		warehouseId: ''
	}
...
const state = reactive<{ visible: boolean; activeTabName: string; stockInNo: string; canChangeWarehouse: boolean; loading: boolean }>({
	visible: false,
	activeTabName: 'base',
	loading: false,
	stockInNo: '',
...
const data = reactive<{ warehouses: Warehouse[]; formdata: EditStockIn }>({
	warehouses: [],
...
const edit = async (id: string) => {
	...
	state.stockInNo = item.stockInNo
	data.formdata = item
}
