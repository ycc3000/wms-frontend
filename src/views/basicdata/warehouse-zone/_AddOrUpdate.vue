	...
	>
		<el-form ref="dataFormRef" :model="state.formdata" :rules="dataRules" label-width="120px">
			<el-form-item prop="warehouseId" label="所属仓库">
				<el-select v-model="state.formdata.warehouseId">
			...
			<el-form-item prop="code" label="存储区编码">
				<el-input v-model="state.formdata.code" placeholder="存储区编码" />
			</el-form-item>
			<el-form-item prop="name" label="存储区名称">
				<el-input v-model="state.formdata.name" placeholder="存储区名称" />
			</el-form-item>
...
import { createOrUpdate } from '@/api/basicdata/warehouse/zone'
import { Warehouse, EditZone } from '@/types/basicdata/warehouse'
...
const getDefaultFormData = (): EditZone => {
	return {
		warehouseId: '',
		code: '',
		name: '',
...
const edit = async (item: EditZone) => {
	if (!item) {
