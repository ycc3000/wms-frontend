	...
	<el-card>
		<template #header>
			<div class="card-header">
				...
				<el-button v-permissions="[PackageTypePermission.Create]" type="primary" @click="addItem()">新增</el-button>
			</div>
		</template>
		<el-table v-loading="state.loading" :data="state.items" table-layout="auto" stripe border>
			...
			<el-table-column v-permissions="PackageTypePermission.CUD" label="操作" fixed="right" class-name="no-wrap">
				<template #default="scope">
					<el-button v-permissions="[PackageTypePermission.Edit]" type="primary" link @click="editItem(scope.row.id)">
						<svg-icon icon="icon-edit" />修改
					</el-button>
					<el-button v-permissions="[PackageTypePermission.Delete]" type="danger" link @click="deleteItem(scope.row.id)">
...
import { PackageTypePermission } from '@/permissions/basicdata'

const addOrUpdateRef = ref()

const state: IHooksOptions = reactive({
	getListUrl: '/api/basic-data/package-type/list',
	getItemUrl: '/api/basic-data/package-type/get-by-id',
	deleteUrl: '/api/basic-data/package-type/delete',
	paging: true,
	query: {
		keyword: ''
	}
