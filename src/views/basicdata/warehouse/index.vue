	...
	<el-card>
		<template #header>
			<div class="card-header">
				...
				<el-button v-permissions="[WarehousePermission.Create]" type="primary" @click="addItem()">新增仓库</el-button>
			</div>
		</template>
		<el-table v-loading="state.loading" :data="state.items" table-layout="auto" stripe border>
			<el-table-column prop="code" label="仓库编码" class-name="no-wrap" />
			<el-table-column prop="name" label="仓库名称" class-name="no-wrap" />
			...
			<el-table-column v-permissions="WarehousePermission.CUD" label="操作" fixed="right" class-name="no-wrap">
				<template #default="scope">
					<el-button v-permissions="[WarehousePermission.Edit]" type="primary" link @click="editItem(scope.row.id)">
						<svg-icon icon="icon-edit" />修改
					</el-button>
					<el-button v-permissions="[WarehousePermission.Delete]" type="danger" link @click="deleteItem(scope.row.id)">
...
import { WarehousePermission } from '@/permissions/basicdata'

const addOrUpdateRef = ref()

const state: IHooksOptions = reactive({
	getListUrl: '/api/basic-data/warehouse/list',
	getItemUrl: '/api/basic-data/warehouse/get-by-id',
	deleteUrl: '/api/basic-data/warehouse/delete',
	paging: true,
	query: {
		keyword: ''
	}
